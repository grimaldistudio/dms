    ...
    public function run($args)
    {
        ...
        foreach($results as $result)
        {
            ...
            if($apiClient->sendRequest('POST', '/noticeboard/api/syncdocument', $data, Yii::app()->params['api_username'], Yii::app()->params['api_password']))
            {
                ...
                try{
                    Yii::app()->db->createCommand("UPDATE documents SET is_dirty = 0, publication_status = :published_status WHERE id=:id")->execute(array(':published_status'=>$result['publication_requested'], ':id'=>$result['id']));
                }
                catch(Exception $e)
                {
