...
class PreviewManager {
    ...
    private function generatePreview($page)
    {
        ...
        try{
            $pdf = $this->path.'['.$page.']';
            exec("convert -geometry ".self::FULL_MAX_WIDTH."x".self::FULL_MAX_HEIGHT." -density 200x200 -quality 100 ".escapeshellarg($pdf)." ".escapeshellarg($this->getPagedCachePath($page)));
            return true;
        }
        catch(Exception $e)
        {
    ...
    private function generateThumbnail()
    {
        try{
            $pdf = $this->path.'[0]';
            exec("convert -geometry ".self::THUMBNAIL_MAX_WIDTH."x".self::THUMBNAIL_MAX_HEIGHT." -density 200x200 -quality 100 ".escapeshellarg($pdf)." ".escapeshellarg($this->getThumbnailCachePath()));
            return true;
        }
        catch(Exception $e)
        {
